...
def ingest_book(file_path: str, reset: bool = False, enrich: bool = False, provider: str = "anthropic", threads: int = 5):
    ...
    register_book(book_meta, chunk_count=len(embedded_chunks), enriched=enrich)
    print(f"Concluído! Livro '{book_meta.get('title')}' disponível com book_id: {book_id}")


if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Uso: python -m scripts.ingest <epub> [--reset] [--enrich] [--provider=anthropic|deepseek|...] [--threads=5]")
        sys.exit(1)
